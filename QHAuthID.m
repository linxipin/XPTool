...
@implementation QHAuthID

+ (instancetype)sharedInstance {
    static QHAuthID *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[QHAuthID alloc] init];
    });
    return instance;
}

- (void)qh_showAuthIDWithDescribe:(NSString *)describe block:(QHAuthIDStateBlock)block {
    if(!describe) {
        if(iPhoneX){
            describe = @"验证已有面容";
        }else{
